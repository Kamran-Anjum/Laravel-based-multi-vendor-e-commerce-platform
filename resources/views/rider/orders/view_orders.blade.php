                                ...
                                @foreach($orders as $order)
                                    ...
                                    <tr class="gradeX">
                                        ...
                                        <td class="center" style="text-align:left;">
                                            <a href="#myModal{{ $order->id }}" data-toggle="modal" style="margin:2px 0px;" class="btn btn-info btn-mini">View</a>
                                            <a href="{{ url('/rider/view-order-location/'.$order->id) }}" style="margin:2px 0px;" class="btn btn-primary btn-mini">View Location</a>
                                            @if($order->status_id == 3)
                                              <a param-id="{{ $order->id }}" param-route="order-accepted" style="margin:2px 0px;" class="btn btn-success btn-mini rider-order-accept">Accept</a>
                                            @elseif($order->status_id == 4 && $order->rider_id == Auth::user()->id)
                                              <a param-id="{{ $order->id }}" param-route="order-delivered" style="margin:2px 0px;" class="btn btn-success btn-mini rider-order-delivered">Deliverd</a>
                                              <a param-id="{{ $order->id }}" param-route="order-cancel"  style="margin:2px 0px;" class="btn btn-danger btn-mini rider-order-cancel">Cancel</a>
                                            @elseif($order->status_id == 5 && $order->rider_id == Auth::user()->id)
                                              <button style="margin:2px 0px;" disabled="" class="btn btn-success btn-mini">Delivered</button>
                                            @endif
                                    ...
                                    <div id="myModal{{ $order->id }}" class="modal hide">
                                        ...
                                        <div class="modal-body">
                                          <style>
                                            ...
                                            <label> <strong>STREET ADDRESS:</strong> {{ $order->streetAddress  }}</label>
                                            <label> <strong>BUILDING ADDRESS:</strong> {{ $order->buildingAddress  }}</label>
