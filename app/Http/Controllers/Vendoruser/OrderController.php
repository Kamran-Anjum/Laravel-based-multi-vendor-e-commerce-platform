...
//            dd($order);
            ...
            return redirect('/vendoruser/view-order')->with('flash_message_success','Order Added Successfully!');
        }
        return view('vendoruser.orders.add_orders');

    }

    public function viewOrder(){

        $vendor_user = VendorUser::where(['user_id'=>Auth::user()->id])->first();

        $orders=DB::table('orders as o')
        ->where(['o.city_id'=>session()->get('VendorUserActiveCity')])
        ->where(['p.created_for'=>$vendor_user->created_by, 'p.lang'=>'en'])
        ...
        return view('vendoruser.orders.view_orders')->with(compact('orders','orderdetails'));
    }
...
//            $orderDetails->save();
            $status = DB::table('status')->where(['deleted_at'=>NULL])->get();
        return view('vendoruser.orders.edit_orders')->with(compact('orderDetail','orderDetails','status'));
    }
