...
class ProductController extends Controller
{
    public function viewProducts(){
        $products = DB::table('product_translates as pt')
        ->where(['pt.created_by'=>Auth::user()->id])
        ->where(['pc.city_id'=>session()->get('VendorUserActiveCity')])
        ->join('product_cities as pc','pc.product_id', '=', 'pt.product_id')
        ->select('pt.*')
        ->orderBy('pt.id','desc')
        ->get();
        ...
        return view('vendoruser.products.view-products')->with(compact('products','categories','brands','product_units','price_units'));
    }
    ...
    public function addProduct(Request $request){
    	if($request->isMethod('post')){
            ...
            $getVendor = DB::table('vendor_users')
            ->where(['user_id'=>Auth::user()->id])
            ->first();

            $vendor_id = $getVendor->created_by;
            ...
            if($product){
                ...
                if($flag == true){
                    if($data['lang_dropdown'] == "en"){
                        ...
                        $en_product_trans->created_for = $vendor_id;
                    ...
                    if($data['lang_dropdown'] == "ar"){
                        ...
                        $ar_product_trans->created_for = $vendor_id;
            ...
            return redirect('/vendoruser/view-products')->with('flash_message_success','Product Added Successfully!');
        }

        // VendorUserPrivilege country
        $countries = DB::table('vendor_user_privileges as vup')
        ->where(['vup.user_id'=> Auth::user()->id])
        ->where(['c.isActive'=> 1])
        ->where(['c.isDeleted'=> 0])
        ->join('countries as c','vup.country_id','=','c.id')
        ->select('vup.*','c.name as countryName','c.id as countryId')
        ->groupBy('vup.country_id')
        ->get();
        ...
        foreach($en_categories as $en_category){
            ...
            $en_sub_categories = DB::table('category_translates')->where(['parent_id'=>$en_category->id])->where(['isActive' =>'1'])->where(['lang'=>'en'])->get();
        ...
        foreach($ar_categories as $ar_category){
            ...
            $ar_sub_categories = DB::table('category_translates')->where(['parent_id'=>$ar_category->id])->where(['isActive' =>'1'])->where(['lang'=>'ar'])->get();
    	...
    	return view('vendoruser.products.add-product')->with(compact('countries_dropdown','en_categories_dropdown','ar_categories_dropdown','en_brand_dropdown','ar_brand_dropdown','en_productunit_dropdown','ar_productunit_dropdown','en_priceunit_dropdown','ar_priceunit_dropdown'));
    }
    ...
    public function editProduct(Request $request, $id =null){
        if($request->isMethod('post')){
            ...
            return redirect('/vendoruser/view-products')->with('flash_message_success','Product has been Updated Successfully!');
        }
        ...
        return view('vendoruser.products.edit-product')->with(compact('product_trans','en_product_details','ar_product_details','en_categories_dropdown','ar_categories_dropdown','en_brand_dropdown','ar_brand_dropdown','en_productunit_dropdown','ar_productunit_dropdown','en_priceunit_dropdown','ar_priceunit_dropdown'));
    }
    ...
    {
        $cities = DB::table('vendor_user_privileges as vup')
        ->where(['vup.user_id'=> Auth::user()->id])
        ->where(['vup.country_id'=> $id])
        ->where(['ci.isActive'=> 1])
        ->where(['ci.isDeleted'=> 0])
        ->join('cities as ci','vup.city_id','=','ci.id')
        ->select('vup.*','ci.name as cityName','ci.id as cityId')
        ->groupBy('vup.city_id')
        ->get();
...
//         return view('vendoruser.products.search')->with(compact('countries'));
