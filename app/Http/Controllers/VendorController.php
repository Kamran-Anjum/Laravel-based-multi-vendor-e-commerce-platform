...
class VendorController extends Controller
{
    public function viewVendors(Request $request){
        ...
        $vendor_requests = VendorApprovalRequest::get();
    	...
    	return view('admin.vendors.view_vendors')->with(compact('users','vendor_requests','vendor_city_requests','cityReq','approvalReq'));
    }
    ...
    public function viewVendorDetails($id){
    	...
    	return view('admin.vendors.view_vendor_details')->with(compact('vendorDetails'));
    }
    ...
    public function activeVendor($id){
        ...
        $vendor_requests = VendorApprovalRequest::where(['user_id'=>$id])->get();
    ...
    public function updateVendorApprovalRequest(){
        $vendor_requests = VendorApprovalRequest::where(['viewAdmin'=>0])->get();
        if(!$vendor_requests->isEmpty()){
            $update_requests = DB::table('vendor_approval_requests')->update([
                'viewAdmin' => 1
            ]);
        }
    ...
    public function updateVendorCityRequest(){
        $vendor_requests = VendorCityRequest::where(['viewAdmin'=>0])->get();
        if(!$vendor_requests->isEmpty()){
            $update_requests = DB::table('vendor_city_requests')->update([
                'viewAdmin' => 1
            ]);
        }
