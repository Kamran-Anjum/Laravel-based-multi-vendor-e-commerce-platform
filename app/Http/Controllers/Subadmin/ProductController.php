...
class ProductController extends Controller
{
    public function viewProducts(){
        ...
        return view('subadmin.products.view-products')->with(compact('products','categories','brands','product_units','price_units'));
    }
    ...
    public function addProduct(Request $request){
    	if($request->isMethod('post')){
            ...
            return redirect('/subadmin/view-products')->with('flash_message_success','Product Added Successfully!');
        }

        // SubAdminPrivilege country
        ...
        $vendors = DB::table('vendor_privileges as vp')
                    ->where(['vp.city_id'=> session()->get('SubAdminActiveCity')])
                    ->join('vendors as v', 'vp.user_id', '=', 'v.user_id')
    	...
    	return view('subadmin.products.add-product')->with(compact('countries_dropdown','en_categories_dropdown','ar_categories_dropdown','en_brand_dropdown','ar_brand_dropdown','en_productunit_dropdown','ar_productunit_dropdown','en_priceunit_dropdown','ar_priceunit_dropdown', 'vendor_dropdown'));
    }
    ...
    public function editProduct(Request $request, $id =null){
        if($request->isMethod('post')){
            ...
            return redirect('/subadmin/view-products')->with('flash_message_success','Product has been Updated Successfully!');
        }
        ...
        return view('subadmin.products.edit-product')->with(compact('product_trans','en_product_details','ar_product_details','en_categories_dropdown','ar_categories_dropdown','en_brand_dropdown','ar_brand_dropdown','en_productunit_dropdown','ar_productunit_dropdown','en_priceunit_dropdown','ar_priceunit_dropdown'));
    }
    ...
    {
        $cities = DB::table('sub_admin_privileges as sap')
        ->where(['sap.user_id'=> Auth::user()->id])
        ->where(['sap.country_id'=> $id])
        ->where(['ci.isActive'=> 1])
        ->where(['ci.isDeleted'=> 0])
        ->join('cities as ci','sap.city_id','=','ci.id')
        ->select('sap.*','ci.name as cityName','ci.id as cityId')
        ->groupBy('sap.city_id')
        ->get();
